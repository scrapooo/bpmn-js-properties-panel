...
var CAMUNDA_EXECUTION_LISTENER_ELEMENT = 'activiti:ExecutionListener';
var CAMUNDA_TASK_LISTENER_ELEMENT = 'activiti:TaskListener';
var CAMUNDA_EVENT_LISTENER_ELEMENT = 'activiti:EventListener';
...
module.exports = function(element, bpmnFactory, options, translate) {
  ...
  function getSelectedListener(element, node) {
    ...
    if (!listener && eventListenerEntry) {
      selection = eventListenerEntry.getSelected(element, node);
      listener = getListeners(bo, CAMUNDA_EVENT_LISTENER_ELEMENT)[selection.idx];
    }
    return listener;
  }
  ...
  var setOptionLabelValue = function(type) {
    return function(element, node, option, property, value, idx) {
      ...
      if (ImplementationTypeHelper.isEventListener(listener)) {
        option.text = (LISTENER_TYPE_LABEL[listenerType] || '');
      } else {
    ...
    if (bo) {
      ...
      var executionListenerEntry = extensionElementsEntry(element, bpmnFactory, {
        ...
        onSelectionChange: function(element, node, event, scope) {
          ...
          eventListenerEntry && eventListenerEntry.deselect(element,node);
        },
  ...
  if (is(element, 'bpmn:Process')) {
    bo = getBusinessObject(element);
    if (bo) {
      var eventListenerEntry = extensionElementsEntry(element, bpmnFactory, {
        id : 'eventListeners',
        label : translate('Event Listener'),
        modelProperty: 'name',
        idGeneration: 'false',

        createExtensionElement: newElement(element, CAMUNDA_EVENT_LISTENER_ELEMENT),
        removeExtensionElement: removeElement(element, CAMUNDA_EVENT_LISTENER_ELEMENT),

        getExtensionElements: function(element) {
          return getListeners(bo, CAMUNDA_EVENT_LISTENER_ELEMENT);
        },

        onSelectionChange: function(element, node, event, scope) {
          executionListenerEntry && executionListenerEntry.deselect(element, node);
          taskListenerEntry && taskListenerEntry.deselect(element, node);
        },

        setOptionLabelValue: setOptionLabelValue(CAMUNDA_EVENT_LISTENER_ELEMENT)
